...
setwd("/home/marcelo/Documents/stored_GitRepos/Latam-Aging-Meta-Analysis/Figures/Functionality/wo_o")
...
# remove outliers
grouped_data<-subset(grouped_data, !(author %in% c("Borda")) )
Demo<-subset(Demo, !(author %in% c("Mattos", "Santamaría-García" )))
Health<-subset(Health, !(author %in% c("Mattos")) )
Sdoh<- subset(Sdoh, !(author %in% c("Borda" )) )
...
for (i in 1:length(names)) {
    tryCatch({
        ...
        cat("Continuing to the next iteration.\n")
        sink()
        dev.off()
    })
}
...
################ 
##GOSH
modelos <- list(meta_pm, meta_pm_demo, meta_pm_health, meta_pm_mentalh, meta_pm_life, meta_pm_sdoh)

names <- c("Fun_All_Gosh.tiff", "Fun_Demographics_Gosh.tiff","Fun_Health_Gosh.tiff","Fun_Mental_Health_Gosh.tiff","Fun_Lifestyle_Gosh.tiff", "Fun_SDOH_Gosh.tiff")
...
for (i in 1:length(names)){
    ...
    plot(res_gosh, alpha = 0.5, het="I2", col=c("#0d5c0a"), cex.axis = 1.4,cex.lab = 1.5)

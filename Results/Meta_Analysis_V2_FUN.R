...
data <- read_csv("/home/marcelo/Documents/stored_GitRepos/Latam-Aging-Meta-Analysis/Data/Fun.csv")
...
setwd("/home/marcelo/Documents/stored_GitRepos/Latam-Aging-Meta-Analysis/Figures/Functionality")
...
# find.outliers(meta_pm)
# find.outliers(meta_pm_demo)
# find.outliers(meta_pm_health)
# find.outliers(meta_pm_mentalh)
# find.outliers(meta_pm_life)
# find.outliers(meta_pm_sdoh)
...
### Forest plots
names <- c("Fun_All_Forest.tiff", "Fun_Demographics_Forest.tiff", "Fun_Health_Forest.tiff",
           "Fun_Lifestyle_Forest.tiff", "Fun_Mental_Health_Forest.tiff", "Fun_SDOH_Forest.tiff")
...
names <- c("Fun_All_funnel_en.tiff", "Fun_Demographics_funnel_en.tiff","Fun_Health_funnel_en.tiff",
           "Fun_Lifestyle_funnel_en.tiff","Fun_Mental_Health_funnel_en.tiff", "Fun_SDOH_funnel_en.tiff")
...
### Egger's test
# eggers.test(meta_pm)
# eggers.test(meta_pm_demo)
# eggers.test(meta_pm_health)
# eggers.test(meta_pm_mentalh)
# eggers.test(meta_pm_life)
# eggers.test(meta_pm_sdoh)

names <- c("Fun_All_Eggers.txt", "Fun_Demographics_Eggers.txt","Fun_Health_Eggers.txt",
           "Fun_Lifestyle_Eggers.txt","Fun_Mental_Health_Eggers.txt", "Fun_SDOH_Eggers.txt")
...
for (i in 1:length(names)) {
    tryCatch({
        ...
        cat("Error in processing file:", names[i], "\n")
        cat("Error message:", conditionMessage(e), "\n")
    })
}


names <- c("Fun_All_pcurve.tiff", "Fun_Demographics_pcurve.tiff","Fun_Health_pcurve.tiff",
           "Fun_Lifestyle_pcurve.tiff","Fun_Mental_Health_pcurve.tiff", "Fun_SDOH_pcurve.tiff")
...
for (i in 1:length(names)){
    ...
    # Usamos tryCatch específicamente alrededor de la función que causa el error
    result <- tryCatch({
        pcurve(modelos[[i]])
    }, error = function(e) {
        message(paste("Error encontrado en", names[i], ": ", e$message))
        return(NULL)  # Devolvemos NULL para que no se intente imprimir nada
    })
...
################ 
##GOSH
names <- c("Fun_All_Gosh.tiff", "Fun_Demographics_Gosh.tiff","Fun_Health_Gosh.tiff",
           "Fun_Lifestyle_Gosh.tiff","Fun_Mental_Health_Gosh.tiff", "Fun_SDOH_Gosh.tiff")

sink()
# find.outliers(meta_pm_sdoh)
# meta_pm_sdoh$data$author
out <- c(1,6,6,-1,-1,4)
...
for (i in 1:length(names)){
    ...
    if (out[i] == -1) {
        plot(res_gosh, alpha = 0.5, het="I2", col=c("#3199cf"), cex.axis = 1.4,cex.lab = 1.5)
        dev.off()
        
    } else {
        plot(res_gosh, alpha = 0.5, het="I2", out = out[i], col=c("#3199cf","#0d5c0a"), cex.axis = 1.4,cex.lab = 1.5)
        dev.off()
